...

{
  ...
  const attach = (source) => {
    ...
    bands.forEach((band, i) => {
      ...
      filter.gain.value = isNaN(port.dataset[band])
        ? 1
        : Number(port.dataset[band]);
      filter.type =
        {
  ...
  const convert = (target) => {
    if (
      ...
      target.src.startsWith(origin) === false
    ) {
  ...
  port.addEventListener("levels-changed", () =>
    map.forEach((filters) => {
      bands.forEach((band) => {
  ...
  port.addEventListener("mono-changed", () =>
    map.forEach((filters) => {
      const { destination } = filters.preamp.context;
      destination.channelCount =
        port.dataset.mono === "true" ? 1 : destination.maxChannelCount;
    })
  );
